...
def extract_genres(batch, date_from: datetime, date_to: datetime):
    """
    ...
    logging.info(f'Search for modified records in genres table: {query}')
    with psycopg2.connect(dsn=dsn) as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cursor:
            ...
            genres = cursor.fetchmany(batch_size)
            while genres:
                batch.send(genres)
                genres = cursor.fetchmany(batch_size)
